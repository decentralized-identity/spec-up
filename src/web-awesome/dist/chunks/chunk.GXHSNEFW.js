...
var WaCombobox = class extends WebAwesomeFormAssociatedElement {
  ...
  getRealOptions() {
    return this.getAllOptions().filter((o) => !o.hasAttribute("data-create-option"));
  }
  ...
  updateCreateOption() {
    ...
    const query2 = this.inputValue.trim();
    const allRealOptions = this.getRealOptions();
    const exactMatch = allRealOptions.some((option) => option.label.toLowerCase() === query2.toLowerCase());
    if (query2 && !exactMatch) {
      ...
      this.createOptionEl.value = query2;
      this.createOptionEl.textContent = this.localize.term("createOption", query2);
  ...
  updateFilteredOptions({ skipCreateOption = false } = {}) {
    ...
    const query2 = this.inputValue.toLowerCase();
    this.filteredOptions = allOptions.filter((option) => {
      ...
      if (this.filter) {
        ...
        matches = option.label.toLowerCase().includes(query2);
      }
  ...
  render() {
    ...
    const hasClearIcon = (this.hasUpdated || isServer) && this.withClear && !this.disabled && (this.value && (Array.isArray(this.value) ? this.value.length > 0 : true) || this.inputValue);
    return html`
      <div
        part="form-control"
        class=${classMap({

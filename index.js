...
module.exports = function(options = {}) {
  ...
  try {
    ...
    function render(spec, assets) {
      try {
        noticeTitles = {};
        specGroups = {};
        console.log('Rendering: ' + spec.title);
        var features = (({ source, logo }) => ({ source, logo }))(spec);
        var doc = (spec.markdown_paths || ['spec.md']).map(path => {
          return fs.readFileSync(spec.spec_directory + path, 'utf8');
        }).join("\n");
        
        fs.writeFileSync(spec.destination + 'index.html', `
            <!DOCTYPE html>
            <html lang="en">
              <head>
                <meta charset="utf-8">
    ...
    config.specs.forEach(spec => {
      ...
      render.call(null, spec, assetTags);
